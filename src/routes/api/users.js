    ...
    User.findOne({ email: req.body.email }).then((user) => {
        if (user) {
            ...
            const message = compileEjs({ template: 'general-template' })({
                ...
                ctaText: 'Verify',
                ctaLink: `${domain}/api/v1/verify-email?token=${jwtSignature(
                    req.body.email
                )}`,
            })
            sendEmail({
        ...
        if (!user) {
            ...
            const message = compileEjs({ template: 'general-template' })({
              ...
              Please click the link below to complete your reset. If this email was sent by mistake kindly ignore`,
                ...
                ctaLink: `${domain}/api/v1/users/reset-password?token=${jwtSignature(
                    req.body.email,
...
router.put('/users/reset-password', (req, res) => {
    ...
    if (Email.message === 'jwt expired Not valid') {
        res.status(400).json({
            message:
                'Request could not be completed at the moment request for password reset again',
        })
    }
    User.findOne({ email: Email }).then((user) => {
        if (!user) {
                ...
                bcrypt.hash(user.password, salt, (err, hash) => {
                    ...
                    user.save()
                        .then((user) =>
                            res
                                .status(200)
                                .json({ message: 'password was changed', user })
                        )
                        .catch((err) => console.log(err))
                })
                const message = compileEjs({ template: 'general-template' })({
